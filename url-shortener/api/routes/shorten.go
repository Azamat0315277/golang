...
func ShortenURL(c *fiber.Ctx) error {
	...
	// every time a user queries, check if the IP is already in database,
	// if yes, decrement the calls remainning by one, else add the IP to database
	...
	// users may abuse the shortener by shorting the domain's `localhost:3000` itself
	...
	// enforce https
	// all url will be convenient to https before storing in database
	...
	// check if user provided short is already in use
	if val != "" {
	...
	// respond with the url, short, expiry in hours
	resp := response{
		URL:             body.URL,
		CustomShort:     "",
		Expiry:          body.Expiry,
